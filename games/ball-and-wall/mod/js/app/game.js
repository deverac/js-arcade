...
function(stage, 
    ...
    // When running locally (i.e. not from a server), clicking on an image
    // causes a Cross-Origin error. To avoid this, a transparent rectangle
    // is overlaid on the image to handle click events. In EaselJS,
    // transparent pixels do not receive click events, so an invisible
    // 'hit target' is also specified.
    // Ideally, a transparent rectangle can be used, but Chrome has a bug which
    // renders transparent pixels as black; this completely obscures the image
    // below. To work around this bug, a rectangle is drawn, but not filled;
    // this results in a thin black line being drawn around the image.
    var cWidth = stage.stage.canvas.width;
    var cHeight = stage.stage.canvas.height;
    var canvas_cover = new createjs.Shape();
    var strokeWidth = 1;
    canvas_cover.graphics.setStrokeStyle(strokeWidth).beginStroke("#000").drawRect(0, 0, cWidth, cHeight);
    canvas_cover.x = 0;
    canvas_cover.y = 0;
    var gHit = new createjs.Shape();
    gHit.graphics.beginFill("#000").rect(0, 0, cWidth, cHeight);
    canvas_cover.hitArea = gHit;
    ...
    Game.prototype.initEvents = function() {
        ...
        if (window.location.href.startsWith('file://')) {
            canvas_cover.addEventListener('mousedown', this.onStageClick.bind(this));
        } else {
    ...
    Game.prototype.update = function(event) {
        ...
        stage.update(event);
        stage.stage.setChildIndex(canvas_cover, stage.stage.getNumChildren()-1);
    };
    ...
    // Ensure that canvas_cover is 'on top'.
    Game.prototype.addCanvasCover = function() {
        if (window.location.href.startsWith('file://')) {
            stage.add(canvas_cover);
            stage.stage.setChildIndex(canvas_cover, stage.stage.getNumChildren()-1);
        }
    };
