...
function(core, dashboard, preloader, episode) {
    ...
    Dashboard.prototype._build = function() {
        ...
        $.each(episode.getManifest().dashboard.buttons, function(i, obj) {
            ...
            // See game.js for why button_cover is needed when running locally.
            var button_cover = new createjs.Shape();
            ...
            $.each(obj, function(key, value) {
                if ( key == 'event' ) {
                    if (window.location.href.startsWith('file://')) {
                        var oWidth = o.image.width;
                        var oHeight = o.image.height;
                        var strokeWidth = 1;
                        button_cover.graphics.setStrokeStyle(strokeWidth).beginStroke("#000").drawRect(0, 0, oWidth, oHeight);

                        var hit = new createjs.Shape();
                        hit.graphics.beginFill("#000").rect(0, 0, oWidth, oHeight);
                        button_cover.hitArea = hit;

                        if (obj.event != 'clickUser') {
                            button_cover.addEventListener('click', function(evt) {
                                _this.emit(value);
                            });
                        }
                    } else {
                ...
                o[key] = value;
                button_cover[key] = value;
            });
            _this.stage.addChild(o);
            if (obj.type == 'Bitmap' && window.location.href.startsWith('file://')) {
                _this.stage.addChild(button_cover);
            }
        });
