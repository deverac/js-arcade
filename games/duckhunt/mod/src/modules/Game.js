import {BaseTexture, extras, loader, loaders, autoDetectRenderer, Spritesheet, Texture} from 'pixi.js';
// Rename utils to avoid conflicting with 'utils' from '../libs/utils'.
...
import jsonDat from '../../dist/sprites.json';
import {encodedPng} from './encodedPng.js';
...
class Game {
   ...
   */
  constructor(opts) {
    pixi_utils.skipHello(); // Suppress Pixi message in console.
  ...
  load() {
      if (window. location. href.startsWith("file://")) {
          this.loader
            .load(this.onLoadLocal.bind(this));
      } else {
        this.loader
  ...
  // Mimic loading spritesheet when running from file.
  // This loads a base64-encoded PNG (instead of loading via XHR request).
  onLoadLocal() {
    ...
    this.loadImage(ctx, image).then(()=> {

        const baseTexture = new BaseTexture(canvas);
        const sprsheet = new Spritesheet(baseTexture, jsonDat);

        sprsheet.parse( (parsed_textures) => {
            loader.resources[this.spritesheet] = new loaders.Resource('sprites.json', '.');
            loader.resources[this.spritesheet].spritesheet = sprsheet;
            loader.resources[this.spritesheet].textures = parsed_textures;
            loader.resources[this.spritesheet].data = jsonDat;
            loader.resources[this.spritesheet].extension = 'json';
            loader.resources[this.spritesheet].type = loaders.Resource.TYPE.JSON;
