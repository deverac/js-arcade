...
function morelemmings() {
  ...
  lemmings[i] = new Lemming(i, state.level.base.start_pos[curdoor][0], state.level.base.start_pos[curdoor][1]);
  lemming_animate(i);

  lemmings_out++;
  document.getElementById('out').firstChild.nodeValue = 'Out: '+lemmings_out;

    if (++curdoor == state.level.base.start_pos.length) {
        curdoor = 0;
    }
}
...
function lemming_target_sel(e) {
  ...
  if (paused == 0 && i == lemming_targetted && lemmings[i]) {
    ...
    if (state.level.lemming_types[selected_type] == 0) changetype = 0;
    ...
    if (changetype != 0) {
      ...
      state.level.lemming_types[selected_type]--;
      document.getElementById('num_'+selected_type).firstChild.nodeValue = (state.level.lemming_types[selected_type] > 0)? state.level.lemming_types[selected_type]:'';
...
function lemming_dig_init(l) {
  ...
  if (row < 160 && lemming_do['dig'][binlevel[row][col]] != 0) {
...
function lemming_bash_init(l) {
  ...
  if (l.dir == 'r') {
    ...
    do {
      ...
      j = col + i;
      if (lemming_do['bash_r'][binlevel[row+15][j]] != 0) {
    ...
    do {
      ...
      j = col + i;
      if (lemming_do['bash_l'][binlevel[row+9][j]] != 0) {
...
function lemming_block_init(l) {
  ...
  for (var j = 0; j < 14; j++) {
    if (binlevel[row+j][col+2] < 10) binlevel[row+j][col+2] += 10;
    if (binlevel[row+j][col+13] < 10) binlevel[row+j][col+13] += 20;
  }
...
function lemming_mine_init(l) {
  ...
  var row = (l.top / 2) + 16;
  var col = (l.left / 2) + 9;
  if (row < 160 && lemming_do['mine_'+l.dir][binlevel[row][col]] != 0) {
...
function lemming_fall_ani(l) {
  ...
  if ((row > 142 || lemming_do['fall'][binlevel[row+17][col]] != 0) &&
      (row > 141 || lemming_do['fall'][binlevel[row+18][col]] != 0)) {
  ...
  if (lemming_do['fall'][binlevel[row+17][col]] == 0) {
...
function lemming_float_ani(l) {
  ...
  if (row < 143 && lemming_do['fall'][binlevel[row+17][col]] == 0) l.changeAnimation('walk');
...
function lemming_walk_ani(l) {
  ...
  row += 7;
  if (row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) { dy--;
    if (--row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) { dy--;
      if (--row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) { dy--;
        if (--row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) { dy--;
          if (--row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) { dy--;
            if (--row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) { dy--;
              if (--row >= 0 && lemming_do['walk'][binlevel[row][col]] == 0) {
                if (l.climber == 0) {
                  l.changeDirection();
                  return;
                } else {
    ...
    if (++row > 159 || lemming_do['walk'][binlevel[row][col]] != 0) { dy++;
      if (++row > 159 || lemming_do['walk'][binlevel[row][col]] != 0) { dy++;
        if (++row > 159 || lemming_do['walk'][binlevel[row][col]] != 0) { dy++;
          if (++row > 159 || lemming_do['walk'][binlevel[row][col]] != 0) { dy++;
            if (++row > 159 || lemming_do['walk'][binlevel[row][col]] != 0) {
              l.top += 6;
              l.anicounter = 2;
...
function lemming_dig_ani(l) {
  ...
  if (l.curleft == -224 || l.curleft == -480) {
    ...
    if (row > 159 || lemming_do['dig'][binlevel[row][col]] == 0) {
      l.top -= 2;
      l.changeAnimation('walk');
    } else {
      l.top += 2;
...
function lemming_bash_ani(l) {
  ...
  if (l.curleft == -96 || l.curleft == -576) {
    ...
    if (lemming_do[bash][binlevel[row+15][col]] != 0) {
...
function lemming_build_ani(l) {
  ...
  if (l.curleft == l.maxleft) {
    ...
    if (lemming_do['build'][binlevel[row+7][col1]] == 0 ||
        lemming_do['build'][binlevel[row+7][col2]] == 0 ||
        lemming_do['build'][binlevel[row+15][col1]] == 0 ||
        lemming_do['build'][binlevel[row+15][col2]] == 0) {
      l.changeAnimation('walk');
      l.changeDirection();
      return;
    }

    if (lemming_do['block_'+l.dir][binlevel[row+15][col1]] != 0 ||
        lemming_do['block_'+l.dir][binlevel[row+15][col2]] != 0) {
      l.changeDirection();
    }
...
function lemming_mine_ani(l) {
  ...
  if (l.curleft == 0) {
    ...
    var row = (l.top / 2) + 16;
    var col = (l.left / 2) + 9;
    if (row > 159 || lemming_do['mine_'+l.dir][binlevel[row][col]] == 0) l.changeAnimation('fall');
...
function lemming_block_ani(l) {
  ...
  if (lemming_do['fall'][binlevel[row+16][col+9]] != 0) {

    for (var j = 0; j < 14; j++) {
      if (binlevel[row+j][col+2] >= 10) binlevel[row+j][col+2] -= 10;
      if (binlevel[row+j][col+13] >= 20) binlevel[row+j][col+13] -= 20;
    }
...
function lemming_climb_ani(l) {
  ...
  if (l.curleft == l.maxleft) {
    ...
    if (lemming_do['climb'][binlevel[row+3][col+7]] == 0) {
      l.changeDirection();
      l.changeAnimation('fall');
    } else {
      l.top -= 8;
    ...
    if (l.curleft == 0 && lemming_do['climb'][binlevel[row+8][col]] != 0) {
      l.changeAnimation('climbtop');
    } else if (l.curleft == -32 && lemming_do['climb'][binlevel[row+7][col]] != 0) {
      l.top -= 2;
      l.changeAnimation('climbtop');
    } else if (l.curleft == -64 && lemming_do['climb'][binlevel[row+6][col]] != 0) {
      l.top -= 4;
      l.changeAnimation('climbtop');
    } else if (l.curleft == -96 && lemming_do['climb'][binlevel[row+5][col]] != 0) {
      l.top -= 6;
      l.changeAnimation('climbtop');
    } else if (l.curleft == -128 && lemming_do['climb'][binlevel[row+4][col]] != 0) {
      l.top -= 8;
      l.changeAnimation('climbtop');
    }
...
function lemming_drown_ani(l) {
  ...
  if (l.curleft < -96 && lemming_do['drown'][binlevel[row+7][col+7]] != 0) l.top += 2 * (Math.floor(Math.random() * 3) - 1);
  else l.top += 2;
  if (lemming_do['drown'][binlevel[row+8][col+4]] != 0 && lemming_do['drown'][binlevel[row+8][col+11]] != 0) l.left += 2* (Math.floor(Math.random() * 3) - 1);
  else l.left += (lemming_do['drown'][binlevel[row+8][col+4]] == 0? 2:-2);
...
function lemming_special_ani(l,row,col) {
  ...
  try {
    ...
    if (binlevel[row][col] < 5) return false;
...
console.log('gog is '+state.gameOverGuard);
  if (lemmings_out == 0 && num_lemmings == 0) setTimeout('game_over('+state.gameOverGuard+')', 10000);
...
function makehole(which, top, left) {
  ...
  for (var j = 0; j < hole.length && row+hole[j][0] < 160; j++) {
    if (lemming_do['hole'][binlevel[row+hole[j][0]][col+hole[j][1]]] != 0) binlevel[row+hole[j][0]][col+hole[j][1]] -= (binlevel[row+hole[j][0]][col+hole[j][1]] % 10);
  }
