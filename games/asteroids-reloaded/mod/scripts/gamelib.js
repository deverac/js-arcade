...
var GameHandler =
{
   ...
   /**
    * Replacement loadSound.
    * Decodes base64-encoded audio data, rather than using XHR request.
    */
   loadSound: function(url, id)
   {
      ...
      {
        var me = this;
        me.audioContext.decodeAudioData(soundUrlToArrayBuffer(url), function(buffer) {
            me.sounds[id] = buffer;
        });
      }
   },
...
/**
 * Replacement for Game.Util.loadMessages.
 * Loads messages from server-data.js, rather than XHR request from server.
 */
Game.Util.loadMessages = function loadMessages(locale)
{
    var url = "messages-" + locale + ".txt"
    var msgs = messageUrl(url).split("\n");
