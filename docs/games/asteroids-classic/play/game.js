...
COLORS = {
    default:     'white', // asteroids, ship, alien, ships remaining
    shipBullet:  'white',
    alienBullet: 'white',
    text:        'white',
    grid:        'red',
...
Bullet = function () {
  ...
  this.draw = function () {
    if (this.visible) {
      ...
      this.context.moveTo(this.x+1, this.y-1);
      this.context.lineTo(this.x-1, this.y+1);
      this.context.strokeStyle = COLORS.shipBullet;
      this.context.stroke();
      this.context.restore();
    }
  };
...
AlienBullet = function () {
  ...
  this.draw = function () {
    if (this.visible) {
      ...
      this.context.strokeStyle = COLORS.alienBullet;
      this.context.stroke();
      this.context.restore();
    }
  };
...
// preload audio
for (var sfx in SFX_SRC) {
  (function () {
    var audio = SFX_SRC[sfx];
    audio.muted = true;
    audio.play();
